...
#pragma omp parallel for
    ...
    imwrite("/home/quinlan/Learn/StereoMatch/dataset/initial_Teddy.png", depth);
...
#pragma omp parallel for
              ...
              for (int offset = 0; offset <= max_offset; offset++)
              {
                  ...
                  for (int i = y-kernel_size; i <= y+ kernel_size; i+=2)
                  {
                      ...
                      for (int j = x-kernel_size; j <= x+kernel_size; j+=2)
                      {
                          ...
                          //for (int c = 0; c < in1.channels();++c)
                          {
                              ...
                              delta_c2 = fabs(window_left_row_ptr[(j+offset)*channel+0] - left_row_ptr[(x+offset)*channel+0])+
                                        fabs(window_left_row_ptr[(j+offset)*channel+1] - left_row_ptr[(x+offset)*channel+1])+
                                        fabs(window_left_row_ptr[(j+offset)*channel+2] - left_row_ptr[(x+offset)*channel+2]);
                          }
                          delta_c1 = delta_c1 / in1.channels();
                          delta_c2 = delta_c2 / in1.channels();
                          double delta_g = sqrt((i - y) * (i - y) + (j - x) * (j - x));
                          ...
                          //for (int c = 0; c < in1.channels(); ++c)
                          {
                              ...
                              float angle = fabs(angle_right_ptr[j*channel+0] - angle_left_ptr[(j + offset)*channel+0]);
                              if (!(angle >= 0 && angle <= 180))
                                  angle = 2 * 180 - angle;

                              float angle_1 = fabs(angle_right_ptr[j*channel+1] - angle_left_ptr[(j + offset)*channel+1]);
                              if (!(angle_1 >= 0 && angle_1 <= 180))
                                  angle_1 = 2 * 180 - angle_1;

                              float angle_2 = fabs(angle_right_ptr[j*channel+2] - angle_left_ptr[(j + offset)*channel+2]);
                              if (!(angle_2 >= 0 && angle_2 <= 180))
                                  angle_2 = 2 * 180 - angle_2;


                              e = gamma_a* (fabs(amp_right_ptr[j*channel+0] - amp_left_ptr[(j + offset)*channel+0])+
                  ...
                  if(offset==max_offset||x+offset==width-1)
                  {
                      ...
                      for(int i=0;i<5&&i<=width-1-x;i++)
                      {
                          ...
                          if(sum/3 <= 25&&value_asw_right[y][x][i].first-value_asw_right[y][x][0].first<50)
                          {
    ...
    cout << "初步计算时间：" << time_used_1.count() << endl;
      ...
      imwrite("/home/quinlan/Learn/StereoMatch/dataset/error_depth_Teddy.png", depth_err);
